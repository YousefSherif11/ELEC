...
@st.cache_data
def load_data():
    """Load data from kagglehub instead of local CSV"""
    ...
    try:
        ...
        # ── قراءة الملف (انت عارف اسمه بالفعل) ───
        full_path = os.path.join(path, "DatafinitiElectronicsProductsPricingData.csv")
        ...
        # ── تأكيد إن الداتا شغالة ───────────────
        if len(df) > 100:
            return df, True

        else:
            st.warning("⚠️ Data loaded but too small")
...
# ══════════════════════════════════════════════════════════════
    ...
    # ── 2. Merchant Analysis ──────────────────────────────────
    st.markdown('<div class="section-title">🏪 Merchant Analysis</div>', unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    with c3:
    ...
    # ── 3. Sale vs Regular ────────────────────────────────────
    st.markdown('<div class="section-title">🏷️ Sale vs Regular Price</div>', unsafe_allow_html=True)
    c5, c6 = st.columns(2)
    with c5:
    ...
    with c6:
        sale_cnt = df["pricesissale"].map({0: "Regular", 1: "On Sale"}).value_counts().reset_index()
        sale_cnt.columns = ["Status", "Count"]
        fig = px.pie(sale_cnt, names="Status", values="Count",
                     title="Sale vs Non-Sale Distribution",
                     color_discrete_sequence=["#636EFA", "#EF553B"])
        fig.update_layout(**DARK)
        st.plotly_chart(fig, use_container_width=True)

    # ── 4. Condition & Shipping ───────────────────────────────
    st.markdown('<div class="section-title">📦 Condition & Shipping</div>', unsafe_allow_html=True)
    c7, c8 = st.columns(2)
    with c7:
    ...
    # ── 5. Brand Analysis ─────────────────────────────────────
    st.markdown('<div class="section-title">🏷️ Brand Analysis</div>', unsafe_allow_html=True)
    if "brand" in df.columns:
        c9, c10 = st.columns(2)
        with c9:
        ...
        with c10:
            brand_avg = df.groupby("brand")[price_col].mean().sort_values(ascending=False).head(10).reset_index()
            brand_avg.columns = ["Brand", "Avg Price"]
            fig = px.bar(brand_avg, x="Brand", y="Avg Price", text="Avg Price",
    ...
    if "pricesdateseen_year" in df.columns:
        st.markdown('<div class="section-title">📅 Price Trend Over Time</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">🟢 Stock Availability</div>', unsafe_allow_html=True)
    ...
    with c14:
        avail_avg = df.groupby("is_in_stock")[price_col].mean().sort_values(ascending=False).reset_index()
        avail_avg.columns = ["Status", "Avg Price"]
        fig = px.bar(avail_avg, x="Status", y="Avg Price", text="Avg Price",
                     color="Avg Price", color_continuous_scale="Teal",
                     title="Average Price by Stock Status")
        fig.update_traces(texttemplate="$%{text:.0f}", textposition="outside")
        fig.update_layout(**DARK, coloraxis_showscale=False)
        st.plotly_chart(fig, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        pricescondition = st.selectbox(
            "Condition",
            ["New", "Refurbished", "Used", "Unknown"],
            index=0,
            key="pricescondition",
        )
        ...
        is_in_stock  = st.selectbox(
            "Stock Status",
            ["Available", "Out Of Stock", "Unknown", "Discontinued", "Limited/Coming Soon"],
        )
        weight_kg    = st.number_input("Weight (kg)", 0.0, 200.0, 1.0, 0.1)
        pricescurrency = st.selectbox("Currency", ["USD", "CAD", "GBP", "EUR"], index=0)
 
    with col2:
        st.markdown("**📂 Category & Platform**")
        first_category = st.selectbox(
            "Category",
            ["Audio & Speakers", "Cameras", "Computers", "Office & Tools",
             "Other", "Phones & Music Players", "TVs"],
            index=2,
        )
        platform       = st.selectbox("Platform", ["amazon", "bestbuy", "walmart", "bhphotovideo", "other"])
        merchant_clean = st.selectbox("Merchant",  ["Amazon", "Best Buy", "Walmart", "B&H Photo", "Other"])
        shipping_type  = st.selectbox("Shipping Type", ["Free", "Standard", "Unknown", "Paid", "Freight"])
    ...
    with col3:
        ...
        dateadded_year    = st.selectbox("Year Added",         YEARS,  index=3)
        dateadded_month   = st.selectbox("Month Added",        MONTHS, index=0)
        dateadded_day     = st.number_input("Day Added",        1, 31, 1)
 
        dateupdated_year  = st.selectbox("Year Updated",       YEARS,  index=3)
        dateupdated_month = st.selectbox("Month Updated",      MONTHS, index=0)
        dateupdated_day   = st.number_input("Day Updated",      1, 31, 1)
 
        pricesdateseen_year  = st.selectbox("Year Price Seen",  YEARS,  index=3)
        pricesdateseen_month = st.selectbox("Month Price Seen", MONTHS, index=0)
        pricesdateseen_day   = st.number_input("Day Price Seen", 1, 31, 1)
 
    # ── Auto-encoded fields ───────────────────────────────────────────────────
    pricescondition_encoded = condition_map.get(pricescondition, 0)
    merchant_freq_encoded   = merchant_map.get(merchant_clean, 100)
    category_freq_encoded   = category_map.get(first_category, 300)
 
    # ── Shipping & category flags ─────────────────────────────────────────────
    st.markdown("---")
    col_a, col_b = st.columns(2)
 
    with col_a:
        st.markdown("**🚚 Shipping Flags**")
        ship_Free     = int(st.checkbox("Free Shipping",     value=(shipping_type == "Free")))
        ship_Standard = int(st.checkbox("Standard Shipping", value=(shipping_type == "Standard")))
        ship_Unknown  = int(st.checkbox("Unknown Shipping",  value=(shipping_type == "Unknown")))
        ship_Paid     = int(st.checkbox("Paid Shipping",     value=(shipping_type == "Paid")))
        ship_Freight  = int(st.checkbox("Freight",           value=(shipping_type == "Freight")))
 
    with col_b:
        st.markdown("**🏷️ Primary Category Flags**")
        pc_electronics   = int(st.checkbox("Electronics",    value=True))
        pc_apple_carplay = int(st.checkbox("Apple CarPlay",  value=False))
        pc_furniture     = int(st.checkbox("Furniture",      value=False))
        pc_intel_celeron = int(st.checkbox("Intel Celeron",  value=False))
        pc_media         = int(st.checkbox("Media",          value=False))
        pc_siri          = int(st.checkbox("Siri Eyes Free", value=False))
    ...
    all_num = {
        ...
        "ship_Free":                          ship_Free,
        "ship_Freight":                       ship_Freight,
        "ship_Paid":                          ship_Paid,
        "ship_Standard":                      ship_Standard,
        "ship_Unknown":                       ship_Unknown,
        "primarycategories_Electronics":      pc_electronics,
        "primarycategories_Apple CarPlay":    pc_apple_carplay,
        "primarycategories_Furniture":        pc_furniture,
        "primarycategories_Intel Celeron":    pc_intel_celeron,
        "primarycategories_Media":            pc_media,
        "primarycategories_Siri Eyes Free":   pc_siri,
    }
    ...
    all_cat = {
        ...
        "dateadded_month":      month_map[dateadded_month],
        "dateupdated_month":    month_map[dateupdated_month],
        "pricesdateseen_month": month_map[pricesdateseen_month],
    }
 
    input_data = {**{c: all_num.get(c, 0)         for c in NUM_FEATURES},
                  **{c: all_cat.get(c, "Unknown")  for c in CAT_FEATURES}}
    ...
    with info_col:
        ...
        summary = {
            ...
            "Stock":       is_in_stock,
            "Weight (kg)": weight_kg,
            "Year Added":  dateadded_year,
        }
        st.dataframe(
            pd.DataFrame(list(summary.items()), columns=["Attribute", "Value"]),
    ...
    ranges = {
        "Phones & Music Players": (50,  700),
        "Computers":              (100, 1100),
        "Cameras":                (80,  1000),
        "TVs":                    (200, 1100),
        "Audio & Speakers":       (30,  400),
        "Office & Tools":         (20,  350),
        "Other":                  (10,  500),
    }
    ...
    st.dataframe(
        range_df.style.background_gradient(subset=["Midpoint ($)"], cmap="Blues"),
        use_container_width=True, hide_index=True,
    )
...
# ══════════════════════════════════════════════════════════════
    ...
    qa_pairs = [
        ...
        },
        {
            ...
            "a": (
                f"Yes. **Sale items** average **${avg_sale:,.0f}** vs. **${avg_nosale:,.0f}** for regular items "
                f"— a difference of **${sale_diff:,.0f}**. This confirms that the `pricesissale` flag is "
